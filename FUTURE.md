...
- **Build Your Perfect Machine — Tesla-style configurator (planned).** Standalone tool that lets a buyer pick model + frame size + table size + optional equipment, see the real all-in price for that exact configuration, and request a quote. When it ships, **rewrite Ch 13 around the configurator as the primary CTA**: replace the static price table with the configurator embed (or a strong CTA to it), demote the demo and advisor CTAs to secondary, and rewrite the H1 around "Build the machine you've been picturing — see the real price." Until then, Ch 13 carries an honest "transparent pricing · base price below" frame that names options + tax as itemized-at-quote-time. The configurator CTA is currently HTML-commented in the Final-CTA row of 13-ownership.html (search "Build Your Perfect Machine") — uncomment + wire URL when ready.
